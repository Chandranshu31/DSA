...
/** THIS SOLUTION IS ALSO  THE SAME BUT IT WILL TAKE A BIT MORE PROCESSING TIME THAN ABOVE SOLN BECAUSE 
HERE WE R USING STRING WE CREATES A NEW OBJECT REF EVERYTIME WE DO RES+= OPERATION UNLIKE THE SB, WHICH UPDATES THE MEMORY
BUFFER ONLY
class Solution {
    public String frequencySort(String s) {
        int n=s.length();
         ...
         while(!pq.isEmpty()){
            ...
            for(Map.Entry<Character,Integer> entry: map.entrySet()){
                ...
                if(currFreq==freq){ // get the character only if the xurr freq is equal to maxfreq
                    char ch=entry.getKey();
                    for(int i=0;i<freq;i++){
                        result+=ch;  // append the character to result for freq times
                    }
                      map.put(ch,0);  // reduce the freq to zero
                      break;
                }
